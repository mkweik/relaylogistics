...
export function calculateLoad(input: LoadInputs): CalculationResult {
  ...
  const tripDays = Math.max(1, Math.ceil(totalMiles / Math.max(input.milesPerDay || 650, 1)));
  ...
  rows.push({ label: `Trip Days Used (${input.milesPerDay || 650} mi/day)`, value: `${tripDays} day${tripDays === 1 ? "" : "s"}` });
