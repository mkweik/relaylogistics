...
export function calculateLoad(input: LoadInputs): CalculationResult {
  ...
  if (input.driverType === "company") {
    ...
    const chargebacks = camera + tollChargeback;
    const absorbedTolls = input.ownerOperatorChargesTolls ? 0 : tolls;
    const absorbed = absorbedTolls + eld + repairReserveAmount + factoring + dispatchFee;
    ...
    if (tollChargeback > 0) rows.push({ label: "Tolls Charged to Owner-Op", value: `+${money(tollChargeback)}` });
