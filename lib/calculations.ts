...
export function calculateLoad(input: LoadInputs): CalculationResult {
  ...
  const returnMiles = hasReturn ? input.returnLoadedMiles + input.returnDeadheadMiles : 0;
  const returnTolls = hasReturn ? input.returnTolls : 0;
  const outboundMiles = input.originLoadedMiles + input.originDeadheadMiles;
  ...
  const outboundFuel = outboundMiles / Math.max(input.mpg, 1) * input.dieselPrice;
  const returnFuel = hasReturn ? returnMiles / Math.max(input.returnMpg, 1) * input.returnDieselPrice : 0;
  ...
  rows.push({ label: "Return Status", value: input.returnStatus });
  ...
  rows.push({ label: "Total Miles", value: `${Math.round(totalMiles).toLocaleString()} mi` });
  ...
  const decision = relayProfit <= 0 || netProfitPerMile <= 0 ? "REJECT" : relayProfit >= input.minProfit && netProfitPerMile >= input.minNetProfitPerMile ? "BOOK" : "NEGOTIATE";
  ...
  return { totalRevenue, totalDriverFacingRate, totalMiles, grossRevenuePerMile, relayProfit, netProfitPerMile, driverSettlement, decision, breakdown: rows };
}
