...
export default function Home() {
  ...
  return (
    <main className="mx-auto max-w-7xl p-6">
      <div className="mb-6">
        <h1 className="text-3xl font-bold">Relay Load Calculator V3.5</h1>
        <p className="text-slate-600">Separated workflows: quote loads before booking, then enter completed loads by driver for real P&L.</p>
      </div>
      ...
      {tab === "quote" ? (
        <section>
          <WorkflowIntro title="Load Quote" text="Use this before booking a load. Nothing is tied to a driver unless you move it into completed loads." />
          <QuoteOrLoadForm
            input={quoteInput}
            result={quoteResult}
        ...
        <section>
          ...
          <CompletedHeader input={completedInput} setInput={setCompletedInput} driverOptions={driverOptions} />
          <QuoteOrLoadForm
  ...
  return (
    <section className="mb-6 rounded-2xl border bg-white p-5">
      ...
      <div className="grid gap-4 md:grid-cols-5">
        ...
        <SelectField label="Driver Type" value={props.newDriverType} onChange={(v) => props.setNewDriverType(v as DriverType)} options={[["company", "Company Driver"], ["owner", "Owner Operator"]]} />
      ...
      {props.editingDriverId && (
        <div className="mt-5 rounded-2xl border bg-slate-50 p-4">
          ...
          <div className="grid gap-4 md:grid-cols-5">
            ...
            <SelectField label="Driver Type" value={props.editDriverType} onChange={(v) => props.setEditDriverType(v as DriverType)} options={[["company", "Company Driver"], ["owner", "Owner Operator"]]} />
        ...
        <table className="w-full min-w-[760px] text-sm">
          ...
          <tbody>
            {props.drivers.length === 0 ? (
              <tr><td colSpan={5} className="p-3 text-center text-slate-500">No drivers loaded.</td></tr>
            ) : (
...
function CompletedHeader({ input, setInput, driverOptions }: { input: LoadInputs; setInput: React.Dispatch<React.SetStateAction<LoadInputs>>; driverOptions: [string, string][] }) {
  return (
    <section className="mb-6 rounded-2xl border bg-white p-5">
      <h2 className="mb-4 text-xl font-semibold">Completed Load Header</h2>
      <div className="grid gap-4 md:grid-cols-5">
        <SelectField label="Driver" value={input.driverId} onChange={(v) => setInput((p) => ({ ...p, driverId: v }))} options={driverOptions} />
        <DateField label="Load Date" value={input.loadDate} onChange={(v) => setInput((p) => ({ ...p, loadDate: v }))} />
        ...
        <Field label="Broker" value={input.broker} onChange={(v) => setInput((p) => ({ ...p, broker: v }))} />
        <Field label="Load #" value={input.loadNumber} onChange={(v) => setInput((p) => ({ ...p, loadNumber: v }))} />
      </div>
    </section>
  );
}
    ...
    <>
      <section className="mb-6 grid gap-6 lg:grid-cols-2">
        <LoadCard
          ...
          onCurrent={(v) => props.setText("originDriverLocation", v)}
          onPickup={(v) => props.setText("originPickupLocation", v)}
          onDelivery={(v) => props.setText("originDeliveryLocation", v)}
          onRc={(v) => props.setNumber("originActualRate", v)}
          onDriverRate={(v) => props.setNumber("originDriverRate", v)}
          onDeadhead={(v) => props.setNumber("originDeadheadMiles", v)}
          onLoaded={(v) => props.setNumber("originLoadedMiles", v)}
          onTolls={(v) => props.setNumber("originTolls", v)}
          onDiesel={(v) => props.setNumber("dieselPrice", v)}
        ...
        <div>
          ...
          <LoadCard
            ...
            onCurrent={(v) => props.setText("returnDriverLocation", v)}
            onPickup={(v) => props.setText("returnPickupLocation", v)}
            onDelivery={(v) => props.setText("returnDeliveryLocation", v)}
            onRc={(v) => props.setNumber("returnActualRate", v)}
            onDriverRate={(v) => props.setNumber("returnDriverRate", v)}
            onDeadhead={(v) => props.setNumber("returnDeadheadMiles", v)}
            onLoaded={(v) => props.setNumber("returnLoadedMiles", v)}
            onTolls={(v) => props.setNumber("returnTolls", v)}
            onDiesel={(v) => props.setNumber("returnDieselPrice", v)}
      ...
      <section className="mb-6 rounded-2xl border bg-white p-5">
        ...
        <div className="grid gap-4 md:grid-cols-4">
          <NumberField label="Company Driver %" value={input.companyDriverPercent} step="0.01" onChange={(v) => props.setNumber("companyDriverPercent", v)} />
          <NumberField label="Owner Operator %" value={input.ownerOperatorPercent} step="0.01" onChange={(v) => props.setNumber("ownerOperatorPercent", v)} />
          <NumberField label="Dispatch Fee %" value={input.dispatchFeePercent} step="0.01" onChange={(v) => props.setNumber("dispatchFeePercent", v)} />
          <NumberField label="Min Profit / Mile" value={input.minNetProfitPerMile} step="0.01" onChange={(v) => props.setNumber("minNetProfitPerMile", v)} />
          <NumberField label="Origin MPG" value={input.mpg} step="0.1" onChange={(v) => props.setNumber("mpg", v)} />
          <NumberField label="Return MPG" value={input.returnMpg} step="0.1" onChange={(v) => props.setNumber("returnMpg", v)} />
          <NumberField label="Insurance / Week" value={input.insuranceWeekly} onChange={(v) => props.setNumber("insuranceWeekly", v)} />
          <NumberField label="ELD / Month" value={input.eldMonthly} onChange={(v) => props.setNumber("eldMonthly", v)} />
          <NumberField label="Camera / Month" value={input.cameraMonthly} onChange={(v) => props.setNumber("cameraMonthly", v)} />
          <NumberField label="Repair Reserve / Mile" value={input.repairPerMile} step="0.01" onChange={(v) => props.setNumber("repairPerMile", v)} />
          <NumberField label="Factoring %" value={input.factoringPercent} step="0.01" onChange={(v) => props.setNumber("factoringPercent", v)} />
        </div>
    ...
    <section className="mb-6 grid gap-6 lg:grid-cols-2">
      <section className="rounded-2xl border bg-white p-5">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <NumberField label="Repair Amount" value={props.repairAmount} step="0.01" onChange={(v) => props.setRepairAmount(Number(v) || 0)} />
      ...
      <section className="rounded-2xl border bg-white p-5">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <SelectField label="Cost Type" value={props.costType} onChange={(v) => props.setCostType(v as "diesel" | "misc")} options={[["diesel", "Diesel"], ["misc", "Misc Cost"]]} />
          <NumberField label="Amount" value={props.costAmount} step="0.01" onChange={(v) => props.setCostAmount(Number(v) || 0)} />
...
function buildReport(loads: SavedLoad[], repairs: Repair[], costs: ActualCost[]) {
  ...
  const actualDiesel = costs.filter((c) => c.cost_type === "diesel").reduce((sum, cost) => sum + Number(cost.amount || 0), 0);
  ...
  for (const cost of costs) {
    ...
    [daily, weekly, monthly].forEach((group, index) => {
      const key = keys[index];
      group[key] ||= makeBlank();
      if (cost.cost_type === "diesel") group[key].actualDiesel += Number(cost.amount || 0);
      else group[key].miscCosts += Number(cost.amount || 0);
    });
  }
