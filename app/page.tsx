...
export default function Home() {
  ...
  return (
    <main className="mx-auto max-w-7xl p-6">
      ...
      <section className="mb-6 grid gap-6 lg:grid-cols-2"><div className="rounded-2xl border bg-white p-5"><h2 className="mb-4 text-xl font-semibold">Originating Load</h2>
          <div className="mb-4 rounded-xl border bg-slate-50 p-3">
            ...
            <input type="file" accept="application/pdf" onChange={(e) => setOriginRcFile(e.target.files?.[0] || null)} className="mb-3 w-full rounded-xl border bg-white p-3" />
            <div className="flex flex-wrap gap-2">
              <button onClick={() => parseRc(originRcFile, "origin")} className="rounded-xl bg-slate-900 px-3 py-2 text-sm font-semibold text-white">Parse Origin RC</button>
              <button onClick={() => uploadRc(originRcFile, "origin_rc")} className="rounded-xl bg-green-700 px-3 py-2 text-sm font-semibold text-white">Upload Origin RC</button>
            </div>
          </div>
          <div className="grid gap-4 md:grid-cols-2"><Field label="Broker" value={input.broker} onChange={(v) => setText("broker", v)} /><Field label="Load Number" value={input.loadNumber} onChange={(v) => setText("loadNumber", v)} /><NumberField label="Actual Rate Con Value" value={input.originActualRate} onChange={(v) => setNumber("originActualRate", v)} /><NumberField label="Rate Given to Driver" value={input.originDriverRate} onChange={(v) => setNumber("originDriverRate", v)} /><NumberField label="Loaded Miles" value={input.originLoadedMiles} onChange={(v) => setNumber("originLoadedMiles", v)} /><NumberField label="Deadhead Miles" value={input.originDeadheadMiles} onChange={(v) => setNumber("originDeadheadMiles", v)} /><NumberField label="Tolls" value={input.originTolls} onChange={(v) => setNumber("originTolls", v)} /><NumberField label="Diesel Price" value={input.dieselPrice} step="0.01" onChange={(v) => setNumber("dieselPrice", v)} /></div></div>
      ...
      {(parseStatus || uploadStatus) && (
        <section className="mb-6 rounded-2xl border bg-white p-4 text-sm text-slate-700">
          {parseStatus && <div><strong>Parser:</strong> {parseStatus}</div>}
          {uploadStatus && <div><strong>Upload:</strong> {uploadStatus}</div>}
        </section>
      )}
      ...
      <section className="mb-6 rounded-2xl border bg-white p-5">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-slate-600">Latest 25 saved estimates from Supabase. Finally, the app remembers things, unlike half the internet.</p>
          </div>
