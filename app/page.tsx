...
type SavedLoad = {
  ...
  origin_broker?: string | null;
  origin_load_number?: string | null;
  return_load_date?: string | null;
  return_broker?: string | null;
  return_load_number?: string | null;
...
const baseDefaults: LoadInputs = {
  ...
  originLoadDate: today,
  originBroker: "",
  originLoadNumber: "",

  returnLoadDate: today,
  returnBroker: "",
  returnLoadNumber: "",
...
export default function Home() {
  const [tab, setTab] = useState<"quote" | "completed" | "roster">("quote");
  ...
  function buildLoadPayload(input: LoadInputs, result: ReturnType<typeof calculateLoad>) {
    return {
      ...
      load_date: input.originLoadDate || input.loadDate,
      load_number: input.originLoadNumber || input.loadNumber,
      broker: input.originBroker || input.broker,

      origin_load_date: input.originLoadDate || input.loadDate,
      origin_broker: input.originBroker || input.broker,
      origin_load_number: input.originLoadNumber || input.loadNumber,
      return_load_date: input.returnLoadDate || null,
      return_broker: input.returnBroker || null,
      return_load_number: input.returnLoadNumber || null,
  ...
  async function saveCompletedLoad() {
    ...
    if (!completedInput.originLoadDate) {
      setStatus("Enter origin load date before saving completed load.");
      return;
    }
  ...
  return (
    <main className="mx-auto max-w-7xl p-6">
      ...
      <div className="mb-6 flex flex-wrap gap-3">
        ...
        <button onClick={() => setTab("roster")} className={`rounded-xl px-4 py-3 font-semibold ${tab === "roster" ? "bg-slate-900 text-white" : "bg-white border"}`}>Driver Roster</button>
      </div>
      ...
      {tab === "quote" ? (
        ...
        <section>
          <WorkflowIntro title="Driver Roster" text="Add, edit, activate, deactivate, and manage driver truck numbers and owner-operator toll settings." />
          <DriverManagement
            drivers={drivers}
  ...
  return (
    <section className="mb-6 rounded-2xl border bg-white p-5">
      ...
      <div className="mb-5 rounded-2xl border bg-slate-50 p-4">
        <CheckboxField
          label="Return Load"
          checked={input.returnStatus !== "none"}
          onChange={(checked) =>
            setInput((p) =>
              checked
                ? { ...p, returnStatus: p.returnStatus === "none" ? "estimated" : p.returnStatus }
                : {
                    ...p,
                    returnStatus: "none",
                    returnDriverLocation: "",
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <div className="rounded-2xl border bg-slate-50 p-4">
          <h3 className="mb-3 text-lg font-semibold">Originating Load Details</h3>
          <div className="grid gap-4 md:grid-cols-3">
            <DateField
              label="Origin Load Date"
              value={input.originLoadDate}
              onChange={(v: string) => setInput((p) => ({ ...p, originLoadDate: v, loadDate: v }))}
            />
            <Field
              label="Origin Broker"
              value={input.originBroker}
              onChange={(v: string) => setInput((p) => ({ ...p, originBroker: v, broker: v }))}
            />
            <Field
              label="Origin Load #"
              value={input.originLoadNumber}
              onChange={(v: string) => setInput((p) => ({ ...p, originLoadNumber: v, loadNumber: v }))}
            />
          </div>
        </div>
        ...
        <div className="rounded-2xl border bg-slate-50 p-4">
          <h3 className="mb-3 text-lg font-semibold">Return Load Details</h3>
          <div className="grid gap-4 md:grid-cols-3">
            <DateField
              label="Return Load Date"
              value={input.returnLoadDate}
              onChange={(v: string) => setInput((p) => ({ ...p, returnLoadDate: v }))}
            />
            <Field
              label="Return Broker"
              value={input.returnBroker}
              onChange={(v: string) => setInput((p) => ({ ...p, returnBroker: v }))}
            />
            <Field
              label="Return Load #"
              value={input.returnLoadNumber}
              onChange={(v: string) => setInput((p) => ({ ...p, returnLoadNumber: v }))}
            />
          </div>
        </div>
        )}
  ...
  return (
    <>
      <section className="mb-6 grid gap-6 lg:grid-cols-2">
        ...
        <div>
          {!props.showDriverFields && (
          <div className="mb-3 rounded-2xl border bg-white p-4">
            <CheckboxField
              label="Return Load"
              checked={input.returnStatus !== "none"}
              onChange={(checked) => props.setReturnLoadEnabled(checked)}
            />
            <p className="mt-2 text-sm text-slate-600">
              Leave unchecked if there is no return load. Return fields will stay hidden and will not affect calculations.
            </p>
          </div>
          )}
