...
export default function Home() {
  ...
  return (
    <main className="mx-auto max-w-7xl p-6">
      <div className="mb-6">
        <h1 className="text-3xl font-bold">Relay Load Calculator V3.5.1</h1>
...
function CompletedHeader({ input, setInput, driverOptions }: { input: LoadInputs; setInput: React.Dispatch<React.SetStateAction<LoadInputs>>; driverOptions: [string, string][] }) {
  return (
    <section className="mb-6 rounded-2xl border bg-white p-5">
      ...
      <div className="grid gap-4 md:grid-cols-5">
        <SelectField label="Driver" value={input.driverId} onChange={(v: string) => setInput((p) => ({ ...p, driverId: v }))} options={driverOptions} />
        <DateField label="Load Date" value={input.loadDate} onChange={(v: string) => setInput((p) => ({ ...p, loadDate: v }))} />
        <SelectField label="Driver Type" value={input.driverType} onChange={(v: string) => setInput((p) => ({ ...p, driverType: v as DriverType }))} options={[["company", "Company"], ["owner", "Owner-Op"]]} />
    ...
    <>
      ...
      <section className="mb-6 rounded-2xl border bg-white p-5">
        ...
        <div className="grid gap-4 md:grid-cols-4">
          <NumberField label="Company Driver %" value={input.companyDriverPercent} step="0.01" onChange={(v: string) => props.setNumber("companyDriverPercent", v)} />
          <NumberField label="Owner Operator %" value={input.ownerOperatorPercent} step="0.01" onChange={(v: string) => props.setNumber("ownerOperatorPercent", v)} />
