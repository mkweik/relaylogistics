...
export default function Home() {
  ...
  async function updateDriver() {
    ...
    if (!editDriverName.trim()) {
      setStatus("Driver name cannot be blank.");
      return;
    }
  ...
  function findDriverByNameOrTruck(value: string) {
    ...
    return drivers.find((driver) => {
      ...
      const truck = (driver.truck_number || "").toLowerCase();
      return name === search || truck === search;
    });
  }

  function parseBulkDieselText() {
    const rows = bulkDieselText
      .split(/\r?\n/)
      .map((line) => line.trim())
      .filter(Boolean);

    const parsed: { driver_id: string; driver_name: string; truck_number: string | null; cost_date: string; amount: number; vendor: string; description: string }[] = [];
    const errors: string[] = [];
    ...
    rows.forEach((line, index) => {
      ...
      if (!cost_date) {
        errors.push(`Row ${rowNumber}: invalid date "${parts[0]}".`);
        return;
      }

      if (!driver) {
        errors.push(`Row ${rowNumber}: could not match driver/truck "${parts[1]}".`);
        return;
      }

      if (!amount || amount <= 0) {
        errors.push(`Row ${rowNumber}: invalid amount "${parts[2]}".`);
        return;
      }
    ...
    if (errors.length === 0) {
      setStatus(`Parsed ${parsed.length} diesel transaction(s).`);
    } else {
      setStatus(`Parsed ${parsed.length} transaction(s) with ${errors.length} issue(s).`);
    }
  }
  ...
  async function runReport() {
    ...
    if (!costResponse.ok) {
      setStatus(`Report cost fetch failed: ${costData.error}`);
      return;
    }
  ...
  return (
    <main className="mx-auto max-w-7xl p-6">
      <div className="mb-6">
        <h1 className="text-3xl font-bold">Relay Load Calculator V3.4</h1>
        <p className="text-slate-600">Driver profitability, truck numbers, actual repairs, bulk diesel entry, misc costs, and API-powered route costs.</p>
      </div>
      ...
      <section className="mb-6 rounded-2xl border bg-white p-5">
        ...
        {editingDriverId && (
          <div className="mt-5 rounded-2xl border bg-slate-50 p-4">
            ...
            <div className="grid gap-4 md:grid-cols-5">
              <Field label="Driver Name" value={editDriverName} onChange={setEditDriverName} />
              <Field label="Truck #" value={editDriverTruck} onChange={setEditDriverTruck} />
              <SelectField label="Driver Type" value={editDriverType} onChange={(v) => setEditDriverType(v as DriverType)} options={[["company", "Company Driver"], ["owner", "Owner Operator"]]} />
              <CheckboxField label="Active" checked={editDriverActive} onChange={setEditDriverActive} />
              <div className="flex items-end gap-2">
                <button onClick={updateDriver} className="rounded-xl bg-green-700 px-4 py-3 font-semibold text-white">Save</button>
                <button onClick={cancelEditDriver} className="rounded-xl bg-slate-200 px-4 py-3 font-semibold">Cancel</button>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="mt-5 overflow-x-auto rounded-xl border">
          <table className="w-full min-w-[760px] text-sm">
            ...
            <tbody>
              {drivers.length === 0 ? (
                <tr><td colSpan={5} className="p-3 text-center text-slate-500">No drivers loaded yet.</td></tr>
              ) : (
                drivers.map((driver) => (
                  <tr key={driver.id} className="border-t">
                    ...
                    <td className="p-3 text-right">
                      <button onClick={() => startEditDriver(driver)} className="rounded-lg bg-slate-900 px-3 py-2 text-xs font-semibold text-white">Edit</button>
                    </td>
                  </tr>
                ))
              )}
      ...
      <section className="mb-6 rounded-2xl border bg-white p-5">
        ...
        <div className="grid gap-4 md:grid-cols-5">
          <SelectField label="Driver" value={input.driverId} onChange={(v) => setInput((p) => ({ ...p, driverId: v }))} options={[["", "Select Driver"], ...drivers.filter((d) => d.is_active).map((d) => [d.id, `${d.name}${d.truck_number ? " - Truck " + d.truck_number : ""}`] as [string, string])]} />
      ...
      <section className="mb-6 grid gap-6 lg:grid-cols-2">
        <section className="rounded-2xl border bg-white p-5">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <SelectField label="Driver" value={repairDriverId} onChange={setRepairDriverId} options={[["", "Select Driver"], ...drivers.filter((d) => d.is_active).map((d) => [d.id, `${d.name}${d.truck_number ? " - Truck " + d.truck_number : ""}`] as [string, string])]} />
        ...
        <section className="rounded-2xl border bg-white p-5">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <SelectField label="Driver" value={costDriverId} onChange={setCostDriverId} options={[["", "Select Driver"], ...drivers.filter((d) => d.is_active).map((d) => [d.id, `${d.name}${d.truck_number ? " - Truck " + d.truck_number : ""}`] as [string, string])]} />
            <DateField label="Cost Date" value={costDate} onChange={setCostDate} />
            <SelectField label="Cost Type" value={costType} onChange={(v) => setCostType(v as "diesel" | "misc")} options={[["diesel", "Diesel"], ["misc", "Misc Cost"]]} />
            <NumberField label="Amount" value={costAmount} step="0.01" onChange={(v) => setCostAmount(Number(v) || 0)} />
            <Field label="Truck #" value={costTruck} onChange={setCostTruck} />
            <Field label="Vendor" value={costVendor} onChange={setCostVendor} />
            <Field label="Description" value={costDescription} onChange={setCostDescription} />
          </div>
          <button onClick={saveActualCost} className="mt-4 rounded-xl bg-green-700 px-4 py-2 font-semibold text-white">Save Actual Cost</button>
        </section>

        <section className="rounded-2xl border bg-white p-5">
          <h2 className="mb-4 text-xl font-semibold">Bulk Diesel Entry</h2>
          <p className="mb-3 text-sm text-slate-600">
            Paste one diesel transaction per line: date, driver name or truck #, amount, vendor, description.
          </p>
          <textarea
            value={bulkDieselText}
            onChange={(e) => setBulkDieselText(e.target.value)}
          ...
          <div className="mt-3 flex flex-wrap gap-3">
            <button onClick={parseBulkDieselText} className="rounded-xl bg-slate-900 px-4 py-2 font-semibold text-white">Preview Diesel Entries</button>
            <button onClick={saveBulkDiesel} className="rounded-xl bg-green-700 px-4 py-2 font-semibold text-white">Save Previewed Entries</button>
          </div>

          {bulkDieselErrors.length > 0 && (
            <div className="mt-4 rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-800">
              <div className="mb-2 font-semibold">Fix these rows before saving:</div>
              {bulkDieselErrors.map((error) => (
                <div key={error}>{error}</div>
              ))}
            </div>
          )}

          {bulkDieselPreview.length > 0 && (
            <div className="mt-4 overflow-x-auto rounded-xl border">
              <table className="w-full min-w-[680px] text-sm">
                ...
                <tbody>
                  {bulkDieselPreview.map((entry, index) => (
      ...
      <section className="mb-6 rounded-2xl border bg-white p-5">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <SelectField label="Driver" value={reportDriverId} onChange={setReportDriverId} options={[["", "Select Driver"], ...drivers.filter((d) => d.is_active).map((d) => [d.id, `${d.name}${d.truck_number ? " - Truck " + d.truck_number : ""}`] as [string, string])]} />
          ...
          <div className="mt-5 grid gap-3 md:grid-cols-3">
            <Metric label="Saved Load Profit" value={currency(report.loadProfit)} />
            <Metric label="Estimated Diesel Added Back" value={currency(report.estimatedDiesel)} />
            <Metric label="Repair Reserve Added Back" value={currency(report.repairReserve)} />
            <Metric label="Actual Diesel" value={currency(report.actualDiesel)} />
            <Metric label="Actual Repairs" value={currency(report.actualRepairs)} />
            <Metric label="Misc Costs" value={currency(report.miscCosts)} />
            <Metric label="Adjusted Net" value={currency(report.adjustedNet)} />
          </div>
        </section>
...
function buildReport(loads: SavedLoad[], repairs: Repair[], costs: ActualCost[]) {
  ...
  const estimatedDiesel = loads.reduce((sum, load) => sum + Number(load.estimated_diesel_cost || 0), 0);

  const actualRepairs = repairs.reduce((sum, repair) => sum + Number(repair.amount || 0), 0);
  const actualDiesel = costs
    .filter((cost) => cost.cost_type === "diesel")
    .reduce((sum, cost) => sum + Number(cost.amount || 0), 0);
  const miscCosts = costs
    .filter((cost) => cost.cost_type === "misc")
    .reduce((sum, cost) => sum + Number(cost.amount || 0), 0);
  ...
  for (const cost of costs) {
    ...
    groups.forEach((group, index) => {
      ...
      if (cost.cost_type === "diesel") {
        group[key].actualDiesel += Number(cost.amount || 0);
      } else {
...
function SummaryTable({ title, data }: { title: string; data: Record<string, { revenue: number; loadProfit: number; repairReserve: number; estimatedDiesel: number; actualRepairs: number; actualDiesel: number; miscCosts: number; adjustedNet: number; loads: number }> }) {
