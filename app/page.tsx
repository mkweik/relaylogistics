...
export default function Home() {
  ...
  async function calculateTolls(kind: "origin" | "return") {
    ...
    if (!response.ok) {
      setApiStatus(`Tolls not ready: ${data.error}`);
      return;
    }
    ...
    setApiStatus(`Calculated ${kind} tolls.`);
  }
  ...
  return (
    <main className="mx-auto max-w-7xl p-6">
      <div className="mb-6">
        ...
        <p className="text-slate-600">V1.9 skips RC parsing and focuses on route miles, tolls, diesel, and load profit.</p>
      </div>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {(["company", "owner"] as DriverType[]).map((type) => (
            <button
              ...
              className={`rounded-2xl border p-4 text-left ${input.driverType === type ? "border-slate-950 bg-slate-100" : "bg-white"}`}
            >
              <div className="font-bold">{type === "company" ? "Company Driver" : "Owner Operator"}</div>
              <p className="text-sm text-slate-600">
                {type === "company" ? "35% driver pay. Relay pays operating costs." : "85% owner-op settlement. Owner-op always pays diesel."}
              </p>
            </button>
          ))}
        </div>
      ...
      {apiStatus && <section className="mb-6 rounded-2xl border bg-white p-4 text-sm text-slate-700"><strong>Status:</strong> {apiStatus}</section>}
        ...
        <div className="overflow-x-auto rounded-xl border">
          <table className="w-full min-w-[900px] border-collapse text-sm">
            ...
            <tbody>
              {savedLoads.length === 0 ? (
                <tr>
                  <td colSpan={10} className="p-4 text-center text-slate-500">No saved loads yet.</td>
                </tr>
              ) : (
                savedLoads.map((load) => (
                  <tr key={load.id} className="border-b last:border-b-0">
                    ...
                    <td className="p-3 text-right">
                      <button onClick={() => loadSavedEstimate(load)} className="rounded-lg bg-slate-900 px-3 py-2 text-xs font-semibold text-white">Load</button>
                    </td>
                  </tr>
                ))
              )}
  ...
  return (
    <div className="rounded-2xl border bg-white p-5">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Field label="Driver Current Location / ZIP" value={driverLocation} onChange={onDriverLocation} />
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        <NumberField label="RC Rate" value={rcRate} onChange={onRcRate} />
        <NumberField label="Driver Rate" value={driverRate} onChange={onDriverRate} />
      </div>
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        <NumberField label="Tolls" value={tolls} step="0.01" onChange={onTolls} />
        <NumberField label="Diesel Price" value={dieselPrice} step="0.01" onChange={onDieselPrice} />
      </div>

      <div className="mt-4 flex flex-wrap gap-2">
        <button onClick={onCalculateMiles} className="rounded-xl bg-slate-900 px-3 py-2 text-sm font-semibold text-white">Calculate Miles</button>
        <button onClick={onCalculateTolls} className="rounded-xl bg-slate-900 px-3 py-2 text-sm font-semibold text-white">Calculate Tolls</button>
        <button onClick={onRefreshDiesel} className="rounded-xl bg-slate-900 px-3 py-2 text-sm font-semibold text-white">Refresh Diesel</button>
        <button onClick={onCalculateAll} className="rounded-xl bg-green-700 px-3 py-2 text-sm font-semibold text-white">Calculate All</button>
      </div>
    </div>
  );
}
...
function Field({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
  return (
    <label className="block">
      ...
      <input value={value} onChange={(e) => onChange(e.target.value)} className="w-full rounded-xl border p-3" placeholder="Full address or ZIP" />
    </label>
  );
}
