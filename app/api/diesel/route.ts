...
export async function GET(request: Request) {
  try {
    ...
    const region = searchParams.get("region") || "midwest";
    const series = SERIES_BY_REGION[region] || SERIES_BY_REGION.midwest;

    if (!apiKey) {
      return NextResponse.json({ error: "Missing EIA_API_KEY.", fallbackPrice: 5.35 }, { status: 400 });
    }

    const url = `https://api.eia.gov/v2/seriesid/${series}?api_key=${apiKey}`;
    ...
    return NextResponse.json({ error: "Could not fetch diesel price.", fallbackPrice: 5.35 }, { status: 500 });
  }
}
