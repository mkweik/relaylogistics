...
async function getMiles(from: string, to: string, apiKey: string) {
  ...
  if (!response.ok) {
    throw new Error(data?.error?.message || "Google Routes API error.");
  }
...
export async function POST(request: Request) {
  ...
  if (!body.from || !body.to) {
    return NextResponse.json({ error: "Missing from/to address or ZIP." }, { status: 400 });
  }
  ...
  try {
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "Could not calculate route miles." }, { status: 500 });
  }
}
